...
pub fn get_consumer_list(ds: &DataFrame) -> Result<Vec<String>,APIError> {
    ...
    let ds = ds.lazy().group_by(
        [col(COL_CONSUMER_APP_NAME_CONSUMER_FILE)]).agg(
        [col(COL_CONSUMER_APP_NAME_CONSUMER_FILE).count().alias("Count")]
    ).sort([COL_CONSUMER_APP_NAME_CONSUMER_FILE],
    ...
    map_single_result(&mut consumer_list, ds,IDX_COL_CONSUMER_APP_NAME_CONSUMER_FILE);
    Ok(consumer_list)
}
